 ...
 * @author Josh Bloch
 */
public class ComparableTimSort<T extends Comparable<? super T>> {
	/**
	...
	private ComparableTimSort(T[] a, T[] work, int workBase, int workLen) {
		this.a = a;
		...
		if (work == null || workLen < tlen || workBase + tlen > work.length) {
			//tmp = (T[]) new Object[tlen]; // original code, Buis changed to:
			tmp = (T[])java.lang.reflect.Array.newInstance(a.getClass().getComponentType(), tlen);
			tmpBase = 0;
			tmpLen = tlen;
		} else {
			tmpLen = workLen;
	...
	public static <T extends Comparable<? super T>> void sort(T[] a, int lo, int hi,
		 ...
		 */
		ComparableTimSort<T> ts = new ComparableTimSort<T>(a, work, workBase,
				workLen);
	...
	private static <T extends Comparable<? super T>> void binarySort(T[] a,
	 ...
	 *         specified array
	 */
	private static <T extends Comparable<? super T>> int countRunAndMakeAscending(
			T[] a, int lo, int hi) {
		assert lo < hi;
		int runHi = lo + 1;
		if (runHi == hi)
	 ...
	 */
	private static <T extends Comparable<? super T>> int gallopLeft(T key,
			T[] a, int base, int len, int hint) {
	 ...
	 * @return the int k, 0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
	 */
	private static <T extends Comparable<? super T>> int gallopRight(T key,
			T[] a, int base, int len, int hint) {
		assert len > 0 && hint >= 0 && hint < len;
	...
	private void mergeHi(int base1, int len1, int base2, int len2) {
		...
		outer: while (true) {
			...
			do {
				assert len1 > 0 && len2 > 1;
				count1 = len1
						- gallopRight(tmp[cursor2], a, base1, len1, len1 - 1);
				if (count1 != 0) {
					dest -= count1;
					cursor1 -= count1;
					len1 -= count1;
	...
	private T[] ensureCapacity(int minCapacity) {
		if (tmpLen < minCapacity) {
			...
			//T[] newArray = (T[]) new Object[newSize];  // original code, Buis changed to:
